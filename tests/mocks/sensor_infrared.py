...
class SensorInfrared:
    def __init__(
        ...
        # Main configuration
        self.config = ConfigInfrared(**{"freq_update_data": 3.0, "param_2": 5})

    def set_config(self, config_data: ConfigInfrared):
    ...
    async def register_sensor_to_main_app(self):
        register_data = RegisterInfo(type=SensorTypesSupported.INFRARED)
    ...
    async def handler_config_messages(self, msg: Msg):
        try:
            config_data = ConfigInfrared(**json.loads(msg.data.decode("utf-8")))
        except Exception as err:
    ...
    def generate_data_mock(self):
        data = {}
        for field_name, model_field in PayloadInfrared.model_fields.items():
            match field_name:
                case "id":
        ...
        self._last_data = PayloadInfrared(**data)
        logger.debug(f"New sensor data generated: {self._last_data}")

        # Parse data with corresponding model
        # values = [value for value in self._last_data]
        # payload_data = PayloadInfrared(**{"id": self._id, "values": values})
    ...
    async def publish_data(self, payload_data: PayloadInfrared):
